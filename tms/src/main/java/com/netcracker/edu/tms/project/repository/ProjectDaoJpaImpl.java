...
import com.netcracker.edu.tms.task.repository.QueryConsts;
...
public class ProjectDaoJpaImpl implements ProjectDao {
    ...
    public List<UserWithPassword> getTeamByProjectId(BigInteger projectId) {
        ...
        List<UserWithPassword> ret = query.getResultList();
        LOGGER.debug("getTeamByProjectId called with projectId={}", projectId);
        return ret;
    }

    @Override
    public boolean addUsersToProjects(ProjectTeam toAdd) {
    ...
    public boolean deleteUserFromTeam(UserWithPassword userWithPasswordToDelete, BigInteger projectId) {
        ...
        ProjectTeam same = entityManager.find(ProjectTeam.class, userWithPasswordToDelete.getId());

        if (same != null) {
            entityManager.remove(same);
            LOGGER.debug("deleteUserFromTeam called with {}", userWithPasswordToDelete.getId());
            return true;
        } else {
            LOGGER.warn("No such UserWithPassword with id={} in deleteUserFromTeam appeared", userWithPasswordToDelete.getId());
            return false;
        }
    }
}
